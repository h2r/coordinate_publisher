...
def main():
    ...
    left_arm_origin_pub = rospy.Publisher('left_arm_origin', Point, queue_size=1)
    right_arm_origin_pub = rospy.Publisher('right_arm_origin', Point, queue_size=1)
    left_arm_point_pub = rospy.Publisher('left_arm_point', Point, queue_size=1)
    right_arm_point_pub = rospy.Publisher('right_arm_point', Point, queue_size=1)
    legit_pose_pub = rospy.Publisher('updated_position', Bool, queue_size =1)
    ...
    while not rospy.is_shutdown():
        ...
        if(fill_points(tfl)):
            ...
            left_arm_origin_pub.publish(lo)
            right_arm_origin_pub.publish(ro)
            left_arm_point_pub.publish(lp)
            right_arm_point_pub.publish(rp)
            legit_pose_pub.publish(True)
        else: legit_pose_pub.publish(False)
        rate.sleep()
